...
from .gemini_service import generate_project_analysis
    ...
    try:
        ...
        with transaction.atomic():
            ...
            ProjectAnalysis.objects.create(
                ...
                readiness_interpretation=analysis_data.get('readiness_interpretation', 'Your project shows moderate viability.'),
                risk_level=risk_level,
                confidence_score=int(analysis_data.get('confidence_score', 75)),
                confidence_reasoning=analysis_data.get('confidence_reasoning', 'Standard alignment parameters.'),
        ...
        return JsonResponse({'status': 'error', 'message': 'An unexpected error occurred during AI analysis. Please try again later.'}, status=500)
...
def team_details_view(request):
    ...
    """
    projects = UserProject.objects.all()
    return render(request, 'planner/team_details.html', {
        'projects': projects,
        'active_project': None
    })
...
def what_if_simulate_view(request, pk):
    ...
    analysis = getattr(project, 'analysis', None)
    if not analysis:
        return JsonResponse({'status': 'error', 'message': 'Analysis not found'}, status=404)
    ...
    time_ratio = mod_time / float(project.time_available) if project.time_available > 0 else 1.0
    ...
    new_readiness = (new_skill_score * 10) * 0.40 + (new_time_score * 10) * 0.30 + (new_budget_score * 10) * 0.20 + (10 - new_risk_level) * 10 * 0.10
    ...
    readiness_ratio = new_readiness / float(analysis.readiness_score) if analysis.readiness_score > 0 else 1.0
    
    init_realistic_prob = int(analysis.scenarios.get('realistic', {}).get('probability', 55))
    init_optimistic_prob = int(analysis.scenarios.get('optimistic', {}).get('probability', 80))
    init_pessimistic_prob = int(analysis.scenarios.get('pessimistic', {}).get('probability', 25))
    ...
    new_pessimistic_prob = max(5, min(95, round(init_pessimistic_prob * (2.0 - readiness_ratio))))
    ...
    log.append({
        ...
        'inputs': {'budget': mod_budget, 'time': mod_time, 'team_size': team_size, 'skill_level': skill_level},
    ...
    return JsonResponse({
        ...
        'current': {
            ...
            'category': "Strong Readiness" if analysis.readiness_score >= 71 else ("Moderate Readiness" if analysis.readiness_score >= 41 else "High Risk"),
...
def download_pdf_view(request, pk):
    ...
    # --- PAGE 1: COVER PAGE ---
    story.append(Paragraph("<font size=16 color='#6366f1'><b>⚡ BrainForge AI</b></font>", body_style))
    story.append(Spacer(1, 40))
    story.append(Paragraph("Project Execution &amp; Viability Report", title_style))
    story.append(Paragraph(f"Strategic analysis and milestones roadmap generated for the project: <b>{project.idea_name}</b>", subtitle_style))
    story.append(Spacer(1, 20))
    
    # Cover Metadata Block Table
    readiness_cat = "Strong Readiness" if analysis.readiness_score >= 71 else ("Moderate Readiness" if analysis.readiness_score >= 41 else "High Risk")
    meta_data = [
        [Paragraph("<b>Project Name:</b>", meta_label_style), Paragraph(project.idea_name, meta_value_style)],
        [Paragraph("<b>Core Goal:</b>", meta_label_style), Paragraph(project.goal, meta_value_style)],
        [Paragraph("<b>Weekly Time Budget:</b>", meta_label_style), Paragraph(f"{project.time_available} Hours/Week", meta_value_style)],
        [Paragraph("<b>Financial Budget:</b>", meta_label_style), Paragraph(f"${project.budget} USD", meta_value_style)],
        [Paragraph("<b>Readiness Score:</b>", meta_label_style), Paragraph(f"<b>{analysis.readiness_score}/100</b> ({readiness_cat})", meta_value_style)],
        [Paragraph("<b>Confidence Level:</b>", meta_label_style), Paragraph(f"{analysis.confidence_score}% ({analysis.confidence_level})", meta_value_style)],
        [Paragraph("<b>Generated Date:</b>", meta_label_style), Paragraph(datetime.datetime.now().strftime('%B %d, %Y'), meta_value_style)]
    ]
    ...
    story.append(Paragraph("CONFIDENTIAL | PREPARED BY BRAINFORGE AI PLANNING ENGINE", ParagraphStyle('Conf', parent=body_style, fontName='Helvetica-Oblique', fontSize=8, textColor=colors.HexColor('#94a3b8'))))
    story.append(PageBreak())
    
    # --- PAGE 2: BUSINESS CONCEPT & CLARIFICATION ---
    story.append(Paragraph("1. Executive Summary &amp; Concept Clarification", h1_style))
    ...
    story.append(Paragraph(f"<b>Readiness Score: {analysis.readiness_score}/100</b> (Category: <i>{readiness_cat}</i>)", h2_style))
    story.append(Paragraph(analysis.readiness_interpretation or "No custom assessment available.", body_style))
    
    feas_table = [
        ["Feasibility Factor", "Score", "Venture Advisory Rationale"],
        ["Time Feasibility", f"{analysis.feasibility_score.get('time_score')}/10", analysis.feasibility_score.get('time_rationale')],
        ["Budget Feasibility", f"{analysis.feasibility_score.get('budget_score')}/10", analysis.feasibility_score.get('budget_rationale')],
        ["Skill Feasibility", f"{analysis.feasibility_score.get('skill_score')}/10", analysis.feasibility_score.get('skill_rationale')],
        ["Risk Assessment", f"{analysis.risk_level}/10", "General difficulty index factoring complexity, skill levels and resource availability."]
    ]
    ...
    story.append(Paragraph("The key hypotheses that must be validated to protect execution resources:", body_style))
    
    ass_list = analysis.assumptions or []
    if not ass_list:
        ass_list = [
            {"category": "User", "assumption": "Users will upload resumes regularly.", "risk": "Medium", "explanation": "Users find manual uploads tedious.", "validation_recommendation": "Survey 20 target users."}
        ]
        
    ass_table_data = [["Category", "Hypothesis / Assumption", "Risk", "Explanation", "Validation Recommendation"]]
    ...
    risks_table = [
        ["Risk Area", "Adverse Roadblock / Threat Description", "Proactive Mitigation Strategy"],
        ["Technical", analysis.risks.get('technical', {}).get('description', ''), analysis.risks.get('technical', {}).get('mitigation', '')],
        ["Resource", analysis.risks.get('resource', {}).get('description', ''), analysis.risks.get('resource', {}).get('mitigation', '')],
        ["Market", analysis.risks.get('market', {}).get('description', ''), analysis.risks.get('market', {}).get('mitigation', '')],
        ["Skills Gap", analysis.risks.get('skills', {}).get('description', ''), analysis.risks.get('skills', {}).get('mitigation', '')]
    ]
    ...
    scen_data = [
        ["Scenario Mode", "Success Prob.", "Target Outcome Description", "Main Success Drivers"],
        ["Optimistic", f"{analysis.scenarios.get('optimistic', {}).get('probability')}%", Paragraph(analysis.scenarios.get('optimistic', {}).get('outcome', ''), body_style), Paragraph(", ".join(analysis.scenarios.get('optimistic', {}).get('factors', [])), body_style)],
        ["Realistic", f"{analysis.scenarios.get('realistic', {}).get('probability')}%", Paragraph(analysis.scenarios.get('realistic', {}).get('outcome', ''), body_style), Paragraph(", ".join(analysis.scenarios.get('realistic', {}).get('factors', [])), body_style)],
        ["Pessimistic", f"{analysis.scenarios.get('pessimistic', {}).get('probability')}%", Paragraph(analysis.scenarios.get('pessimistic', {}).get('outcome', ''), body_style), Paragraph(", ".join(analysis.scenarios.get('pessimistic', {}).get('factors', [])), body_style)]
    ]
    ...
    story.append(Paragraph(f"<b>AI Recommendation Confidence: {analysis.confidence_score}% ({analysis.confidence_level})</b>", h2_style))
    story.append(Paragraph(analysis.confidence_reasoning or "Standard analysis based on user inputs consistency.", body_style))
    story.append(Spacer(1, 4))
    story.append(Paragraph("<i>Disclaimer: BrainForge AI provides decision-support insights, not final decisions. Users remain responsible for all execution choices.</i>", ParagraphStyle('Dis', parent=body_style, fontName='Helvetica-Oblique', fontSize=8.5, textColor=colors.HexColor('#64748b'))))
    story.append(Spacer(1, 10))
    
    # Roadmap Area
    story.append(Paragraph("7. 30/60/90 Day Execution Roadmap", h1_style))
    
    story.append(Paragraph("<b>Days 1 - 30 (Milestone: " + analysis.roadmap.get('day_30', {}).get('milestone', '') + ")</b>", h2_style))
    for t in analysis.roadmap.get('day_30', {}).get('tasks', []):
        story.append(Paragraph(f"• {t}", bullet_style))
        
    story.append(Paragraph("<b>Days 31 - 60 (Milestone: " + analysis.roadmap.get('day_60', {}).get('milestone', '') + ")</b>", h2_style))
    for t in analysis.roadmap.get('day_60', {}).get('tasks', []):
        story.append(Paragraph(f"• {t}", bullet_style))
        
    story.append(Paragraph("<b>Days 61 - 90 (Milestone: " + analysis.roadmap.get('day_90', {}).get('milestone', '') + ")</b>", h2_style))
    ...
    story.append(Paragraph("<b>Today's recommended next step:</b>", ParagraphStyle('TodayH', fontName='Helvetica-Bold', fontSize=10, leading=13, textColor=colors.HexColor('#b91c1c'))))
    story.append(Paragraph(f"\"{analysis.first_action}\"", ParagraphStyle('TodayA', parent=body_style, fontName='Helvetica-BoldOblique', fontSize=10.5, leading=14, textColor=colors.HexColor('#0f172a'))))
    ...
    response['Content-Disposition'] = f'attachment; filename="{project.idea_name.replace(" ", "_")}_viability_report.pdf"'
    response.write(pdf)
    return response
...
def load_hackathon_demo_view(request):
    ...
    with transaction.atomic():
        ...
        analysis = ProjectAnalysis.objects.create(
            ...
            feasibility_score={
                "time_score": 8, "time_rationale": "10 hours/week is solid for building a simple wrapper around a parser.",
                "budget_score": 9, "budget_rationale": "$5000 is ample for basic cloud hosting and free tier API usage.",
                "skill_score": 9, "skill_rationale": "Python and ML skills match the parsing backend requirements perfectly.",
                "overall_score": 9, "overall_rationale": "Highly feasible. Constraints are well-matched to the scope of an MVP."
            },
            risks={
                "technical": {"description": "Parsing multi-column PDF layouts accurately.", "mitigation": "Use standard PDF extraction libraries and parse plain text keywords first."},
                "resource": {"description": "Running out of API tokens on third-party parsing services.", "mitigation": "Caching results and implementing local parsing logic for common keywords."},
                "market": {"description": "Low resume uploads if user onboarding takes too long.", "mitigation": "Implement a 1-click drag-and-drop landing page without requiring early signups."},
                "skills": {"description": "Lack of experience in advanced UX/UI styling details.", "mitigation": "Use professional pre-built clean layouts and vanilla styling frameworks."}
            },
            scenarios={
                "optimistic": {"outcome": "Working MVP in 3 weeks, onboarding 150 users in month 1.", "probability": 80, "factors": ["Pre-trained models", "High student network reach"]},
                "realistic": {"outcome": "Prototype ready in 4 weeks, with moderate signup conversion.", "probability": 55, "factors": ["Typical CSS adjustment delays"]},
                "pessimistic": {"outcome": "Difficulty extracting text from complex PDFs delays launch.", "probability": 25, "factors": ["Underestimating parsing edge cases"]}
            },
            roadmap={
                "day_30": {
