...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if UserProject.objects.filter(idea_name=idea_name).exists():
            self.stdout.write(self.style.SUCCESS("Sample project already exists in the database."))
            return
        ...
        ProjectAnalysis.objects.create(
            ...
            scenarios={
                "optimistic": {
                    ...
                    "probability": 75,
                    "factors": ["High demand in local neighborhood hubs", "Active partnerships with community environmental groups"]
                },
                "realistic": {
                    "outcome": "Reach 50 beta testers in 12 weeks. Safe operations, steady growth, container return rate floats at 88%.",
                    "probability": 55,
                    "factors": ["Word of mouth recommendations", "Consistent flyer distribution at local farmer markets"]
                },
                "pessimistic": {
                    "outcome": "Only 12 customers signed up by day 90. Packaging costs eat up budget due to poor return rates (under 60%).",
                    "probability": 20,
                    "factors": ["Fierce competition from standard delivery apps", "High customer attrition due to container deposit friction"]
                }
            },
            roadmap={
                "day_30": {
        ...
        self.stdout.write(self.style.SUCCESS("Sample project and analysis successfully loaded!"))
